...
public class QueueingNodeManager implements NodeManager {
	...
	public void compact() {
		...
		if (compactNode != dirtyNode) {
			Node<?> parent = dirtyNode.parent;
			if (parent == null) {
				// TODO need reference to containing list
			} else if (parent.left == dirtyNode) {
				parent.left = compactNode;
			} else if (parent.right == dirtyNode) {
				parent.right = compactNode;
			}
		}
	}
}
